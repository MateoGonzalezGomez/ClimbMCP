...
class ClimbingPDFServer {
  constructor() {
    this.server = new Server(
      {
        name: 'climbing-pdf-server',
        version: '1.0.0',
      },
      {
        capabilities: {
  ...
  setupHandlers() {
    this.server.setRequestHandler(ListToolsRequestSchema, async () => ({
      tools: [
        ...
        {
          name: 'get_chapter',
          description: 'Get the content of a specific PDF chapter for Claude to read',
          inputSchema: {
            type: 'object',
            properties: {
              chapter_name: {
        ...
        {
          name: 'search_relevant_chapter',
          description: 'Search for the most relevant chapter based on a query',
          inputSchema: {
            type: 'object',
            properties: {
              query: {
                type: 'string',
                description: 'The search query to find relevant chapters',
              },
            },
            required: ['query'],
          },
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      switch (name) {
        ...
        case 'search_relevant_chapter':
          return await this.searchRelevantChapter(args.query);
  ...
  async getChapter(chapterName) {
    try {
      ...
      // This allows Claude to receive the PDF data through the MCP protocol
  ...
  async searchRelevantChapter(query) {
    try {
      const files = await fs.readdir(CHAPTERS_DIR);
      const pdfFiles = files.filter(file => file.toLowerCase().endsWith('.pdf'));
      
      const scoredChapters = await Promise.all(
        pdfFiles.map(async (file) => {
          const metadataPath = path.join(CHAPTERS_DIR, file.replace('.pdf', '.json'));
          let metadata = { title: file, description: '', keywords: [] };
          ...
          // Simple relevance scoring based on query match in title, description, and keywords
      ...
      // Return top 3 most relevant chapters
      ...
      return {
        content: [
          {
            type: 'text',
            text: `Most relevant chapters for query "${query}":\n${JSON.stringify(topChapters, null, 2)}`,
          },
        ],
      };
    } catch (error) {
      return {
        content: [
          {
            type: 'text',
            text: `Error searching chapters: ${error.message}`,
          },
        ],
        isError: true,
      };
  ...
  async run() {
    ...
    console.error('Climbing PDF MCP server running on stdio');
  }
}

const server = new ClimbingPDFServer();
